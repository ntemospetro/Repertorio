...
// Replace each language block
for (const lang of ['de', 'en', 'fr', 'el', 'it', 'ru', 'es']) {
  // Find the end of the language block
  const blockStartRegex = new RegExp(`${lang}:\\s*\\{`);
  const match = blockStartRegex.exec(updatedFile);
  if (!match) continue;
  
  let braceCount = 1;
  let blockEndIndex = match.index + match[0].length;
  
  while (braceCount > 0 && blockEndIndex < updatedFile.length) {
    if (updatedFile[blockEndIndex] === '{') braceCount++;
    if (updatedFile[blockEndIndex] === '}') braceCount--;
    blockEndIndex++;
  }
  
  const block = updatedFile.substring(match.index, blockEndIndex);
  
  let newBlock = block.slice(0, -1).trim(); // remove trailing '}'
  if (newBlock.endsWith(',')) newBlock = newBlock.slice(0, -1);
  newBlock += ',';
  
  for (const [key, trans] of Object.entries(newKeys)) {
    const text = trans[lang] || trans['en'];
    newBlock += `\n    ${key}: '${text.replace(/'/g, "\\'")}',`;
  }
  
  newBlock += '\n  }';
  
  updatedFile = updatedFile.replace(block, newBlock);
}

fs.writeFileSync('src/i18n/translations.ts', updatedFile);
