...
// Extract language objects
const langRegex = /([a-z]{2}):\s*\{([\s\S]*?)\}(?=\s*,?\s*(?:[a-z]{2}:|$))/g;
let match;
const languages = {};
...
for (const [lang, block] of Object.entries(languages)) {
  let newBlock = block.replace(/\s+$/, '');
  if (!newBlock.endsWith(',')) newBlock += ',';
